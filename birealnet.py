...
class BiConv2d(nn.Conv2d):
    ...
    def __init__(self, *kargs, **kwargs):
        ...
        self.scale = nn.Parameter(torch.rand(self.weight.size(0), 1, 1, 1), requires_grad=True)
        
    def forward(self, input):
        # scale = torch.abs(self.MFilters).mean(dim=1, keepdim=True)
        new_weight = self.weight * self.scale
        output = F.conv2d(input, new_weight, self.bias,
